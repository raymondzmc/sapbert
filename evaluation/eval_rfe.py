...
def main(args):
    ...
    json_path = '../training_data/cursa-train-test-seen-unseen.json'

    with open(json_path, 'r') as f:
        json_data = json.load(f)
        train_ids = set([x[1] for x in json_data['DATASET']['TRAIN']])

    path = '../training_data/CuRSA-FIXED-v0-processed-all.pth'
    data = torch.load(path)
    id2synonyms = data['synonyms_names']

    cache_path = './name2vec.pth'
    # if not os.path.exists(cache_path):
    id2vectors = {}
    for k, v in tqdm(id2synonyms.items()):
        vectors = model_wrapper.embed_dense(names=v, show_progress=False, batch_size=32, agg_mode=args.agg_mode)
        id2vectors[k] = vectors
    # torch.save(id2vectors, cache_path)
    # else:
        # id2vectors = torch.load(cache_path)


    test_set = [x for x in data['data'] if x['id'] not in train_ids]
    recalls = [[0, 0, 0], [0, 0, 0]]
    n_examples = 0
    n_multi = 0
    for example in tqdm(test_set):
        text = example['text']
        mention_vector = model_wrapper.embed_dense(names=[text], show_progress=False, batch_size=32, agg_mode=args.agg_mode)
        probs = []
        for k, v in id2vectors.items():
            ...
            probs.append((k, dense_score_matrix.max().item()))

        for entity in example['entities']:
            gt_concept = entity
            # multi_span = example['ismulti']
            multi_span = False
